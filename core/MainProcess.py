...
from core.task.daily_tasks.Wreckage import Wreckage
...
class MainProcess:
    ...
    async def start(self):
        try:
            ...
            self.logging.log(f"线程 [{self.target}] 执行完成: {e}", self.target)
            self.app.stop(self.target)
        except Exception as e:
            self.logging.log(f"线程 [{self.target}] 执行异常: {e}", self.target)
